export const getColorPalette = () => {
  ...
  // console.log("RGB VALUES ARE : ", rgbValues);

  const colorPaletteRGB = quantization(rgbValues, 0); // quant colors

  const orderedByColor = orderByLuminance(colorPaletteRGB);
  // const hslColors = convertRGBtoHSL(orderedByColor);

  const colorPaletteHex = {};
  // for (let i = 1; i <= colorPaletteRGB.length; i++) {
  //   let color = "--gradient-color-" + i;
  //   colorPaletteHex[color] = rgbToHex(colorPaletteRGB[i - 1]);
  // }
  ...
  for (let i = 0; i < orderedByColor.length; i++) {
    ...
    let color = "--gradient-color-" + colorCount;
    colorCount++;
    colorPaletteHex[color] = hexColor;
  }

  // console.log("Initial color palette is : ", colorPaletteHex);
  // console.log("Number of keys : ", Object.keys(colorPaletteHex).length);

  return Object.keys(colorPaletteHex).length < 4
    ? generateMoreShades(colorPaletteHex)
    : colorPaletteHex;
};
...
const generateMoreShades = (colorPaletteHex) => {
  // console.log("Generating More Shades");
  const numEntries = Object.keys(colorPaletteHex).length;

  if (numEntries === 1) {
    const originalColor = colorPaletteHex["--gradient-color-1"];
    const lighterColor2 = lightenColor(originalColor, 20); // Change percentage as needed
    const lighterColor3 = lightenColor(originalColor, 40); // Change percentage as needed
    const lighterColor4 = lightenColor(originalColor, 60); // Change percentage as needed

    // Add lighter shades to the object
    colorPaletteHex["--gradient-color-2"] = lighterColor2;
    colorPaletteHex["--gradient-color-3"] = lighterColor3;
    colorPaletteHex["--gradient-color-4"] = lighterColor4;
  } else if (numEntries === 2) {
    const originalColor1 = colorPaletteHex["--gradient-color-1"];
    const originalColor2 = colorPaletteHex["--gradient-color-2"];
    const lighterColor3 = lightenColor(originalColor1, 20); // Change percentage as needed
    const lighterColor4 = lightenColor(originalColor2, 20); // Change percentage as needed
    ...
    const originalColor1 = colorPaletteHex["--gradient-color-1"];
    const lighterColor4 = lightenColor(originalColor1, 20); // Change percentage as needed

    // Add lighter shade to the object
    colorPaletteHex["--gradient-color-4"] = lighterColor4;
  }

  // console.log("Final color palette is : ", colorPaletteHex);
...
//     // true when hsl color is not black/white/greys
//     if (hslColors[i].h) {
