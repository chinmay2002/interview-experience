  ...
  useEffect(() => {
    const fetchPlacements = async () => {
      const placementsSnapshot = await getDocs(
        query(collection(db, "placements"), where("companyName", "==", "Tata"))
      );
    ...
    <>
      ...
      {placements.map((ind) => {
        return (
          <div
            key={ind.id}
            className="bg-white rounded-lg shadow-xl  p-4 w-[40%]"
          >
            ...
            <div className=" text-center">
              {" "}
              <div className="text-xl font-semibold mb-2">{ind.personName}</div>
